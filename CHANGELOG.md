...
- `src/components/ui/card.tsx` is the CLI's file again (`npx shadcn@latest add card --overwrite`, with the `cn` import repointed at `@/lib/cn` and `CardDescription` kept at `text-base`): the hand-added `asChild` on `CardTitle` is gone, as the Surfaces rule in `docs/ui-conventions.md` says it must be. The auth headings are real headings inside `CardTitle` (`<CardTitle><h1>Log in</h1></CardTitle>`), so "Log in" and "Create your account" keep their role and take the page-heading type `globals.css` gives every `h1` (Fraunces, bold, `text-4xl`, the size of "Your tasks") instead of the 3xl semibold that `CardTitle`'s own classes had leaked onto them through `asChild`, so "Create your account" now wraps to two lines in the card; the two pages drop their `CardDescription` `text-base` overrides, which the primitive's base now carries. `tests/projector-rules.test.ts` pins card.tsx free of `asChild` and both pages on that shape; the auth screenshots are recaptured.
