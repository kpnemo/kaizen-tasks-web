...
| Task list | AI chip          | "Thinking" while pending or running; "N suggestions" (from the summary's `suggestionCount`, a link to the task) when done with suggestions; a "Breakdown failed" chip plus an alert carrying `<aiError>` and a "Retry" button when failed; "Too short to break down", "Hourly limit reached", or "Assistant paused" when skipped |
...
| Interview | chips            | button "Skip this question", button "Finish with what we have" (after the first question), button "Start over", and one button per option named by its own visible text; the recommended option is first and carries a "Recommended" badge                                                                                       |
